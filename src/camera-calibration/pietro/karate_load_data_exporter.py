...
def camera_factory(calib_info: dict, do_average_results = False) -> Camera:
    ...
    if do_average_results == False:
        rvec = np.array(calib_info['rvecs'][0]).reshape(3,1)
    
        tvec = np.array(calib_info['tvecs'][0]).reshape(3,1)
    ...
    pos = -np.dot(rotation_matrix.transpose(),tvec)
    pos1 = -np.matmul(rotation_matrix.T, tvec)
