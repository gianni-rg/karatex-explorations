...
@click.option('--threshold', type=click.FLOAT, required=True, default=50, help='maximum error threshold')
...
def main(input_path,calibration_file,clip_name,start_frame_nbr,end_frame_nbr,ifiles,nformat,threshold,output_folder):
    ...
    for key in cameras_old:
        ...
        cameras[str(cam_id)].tvec = cameras[str(cam_id)].tvec.reshape((3,))
        cameras[str(cam_id)].rvec = cameras[str(cam_id)].rvec.reshape((3,))
        cam_id += 1
    ...
    for i in range(start_frame_nbr,end_frame_nbr):
        multiview_frame = {"1":frames['c1'][i+1],"2":frames['c2'][i+1],"3":frames['c3'][i+1]}
