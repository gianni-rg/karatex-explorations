    ...
    for i in range(num_frames):
        if np.isnan(frames[i]).all():
            ...
            if len(valid_indices) >= 2:  # At least two points are needed for interpolation
                ...
                for point in range(num_points):  # For each of the keypoints
    ...
    for i in range(1, num_frames - 1):
        ...
        for point in range(num_points):
            if (np.linalg.norm(current_frame[point] - prev_frame[point]) > threshold and
                ...
                num_corrected_frames += 1
    return corrected_frames, num_corrected_frames
...
@click.option('--input_folder', type=click.STRING, required=True, default="output_3d_150", help='relative path folder for the output')
...
@click.option('--window_size', type=click.INT, required=True, default=200, help='frame to start from')
...
def main(input_path,clip_name,input_folder,window_size,threshold,output_folder,window_length):
    ...
    # Detect and correct errors
    corrected_frames, num_corrected_frames = detect_and_correct_errors(interpolated_frames)
    ...
    print(f"Corrected Frames: {num_corrected_frames}")
