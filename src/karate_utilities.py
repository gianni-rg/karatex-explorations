...
class Camera:
   def __init__(self,K: Optional[np.array] = None, Rot: Optional[np.array]=None,
                tvec:Optional[np.array] = None,rvec: Optional[np.array]= None,
                pos: Optional[np.array] = None,dist_coeffs: Optional[np.array] = None):
       self.K = K
   ...
   def from_json(self,json_camera: str) -> None:
       self.pos = np.array(json_camera['pos'],dtype=np.float64).reshape(3,)
       ...
       self.tvec = np.array(json_camera['tvec'],dtype=np.float64).reshape(3,)
       self.rvec = np.array(json_camera['rvec'],dtype=np.float64).reshape(3,)
       
       

    

def factory_camera_from_k4a(resolution:list,principal_point:list,focal_length: list, dist_params: list ) -> Camera :
